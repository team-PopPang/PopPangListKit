...
extension Array {
    ...
    /// 배열을 주어진 개수(`count`) 단위로 나누어 2차원 배열로 반환합니다.
    ///
    /// 내부적으로 `chunks(ofCount:)`를 사용해 SubSequence 단위로 나눈 뒤,
    /// 각 요소를 `Array`로 변환하여 최종적으로 `[[Element]]` 형태로 반환합니다.
    ///
    /// - Parameter count: 하나의 chunk에 포함될 요소 개수 (1 이상)
    /// - Returns: `count` 단위로 나뉜 2차원 배열
    ///
    /// - Note:
    /// `chunks(ofCount:)`는 lazy한 view를 반환하지만,
    /// 이 메서드는 `map(Array.init)`을 통해 실제 배열을 생성합니다.
    ///
    /// - Complexity:
    /// O(n)
    ///
    ...
    func chunks(ofCount count: Int) -> [[Element]] {
        // 1. 배열을 count 개씩 나눈 "chunk 뷰" 생성 (아직 실제 배열은 아님, lazy 느낌)
        let chunksOfCountCollection: ChunksOfCountCollection<Self> = chunks(ofCount: count)
        
        // 2. 각 chunk(SubSequence)를 실제 Array로 변환
        //    → 최종적으로 [[Element]] 형태의 2차원 배열 생성
