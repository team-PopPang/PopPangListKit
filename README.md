...
`position`에는 `.top`, `.centeredVertically`, `.bottom`, `.left`, `.centeredHorizontally`, `.right`를 사용할 수 있습니다. 대부분의 세로 목록은 기본값인 `.top`을 사용하면 됩니다.
...
| `ListProxy` | `scrollToTop`, `scrollToSection` | SwiftUI 버튼·액션에서 목록 위치를 명령형으로 변경합니다. `PopPangListProxy`는 deprecated 호환 alias입니다. |
