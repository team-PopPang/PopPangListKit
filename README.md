...
`didScroll`은 현재 위치를 **관찰**하는 API입니다. 버튼 탭처럼 코드에서 위치를 바꾸려면 `@State`의 `PopPangListProxy`를 `PopPangList(proxy:)`에 전달하세요.
...
| `PopPangListProxy` | `scrollToTop`, `scrollToSection` | SwiftUI 버튼·액션에서 목록 위치를 명령형으로 변경합니다. |
...
| 프로그램 스크롤 | `PopPangList(proxy:)`, `scrollToTop`, `scrollToSection` | `@State` Proxy로 특정 section 또는 실제 콘텐츠 최상단으로 이동합니다. |
