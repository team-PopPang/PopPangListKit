...
PopPangListKit은 복잡한 리스트에서 UIKit의 통제력과 SwiftUI의 생산성을 함께 얻기 위해 만든 List DSL입니다.
...
기본적인 리스트는 SwiftUI `List`로 빠르게 구현할 수 있습니다. 카드형 피드처럼 디자인 자유도가 중요한 화면이라면 `ScrollView + LazyVStack`도 좋은 선택입니다.

### 리스트가 복잡해지면 문제가 달라집니다

리스트의 규모가 커지고 interaction이 복잡해지면 화면을 그리는 것만으로는 충분하지 않습니다. Section별 세로·가로·그리드 배치, 동적 높이, 원격 이미지 prefetch, pagination, scroll lifecycle과 실시간 데이터 반영을 한 화면에서 함께 다루게 됩니다.

이 조건들이 겹치면 데이터와 셀이 갱신되는 과정을 예측할 수 있어야 합니다. 어떤 데이터가 바뀌었는지, 어떤 셀을 다시 그릴지, 스크롤 중인 화면에 언제 반영할지를 통제하지 못하면 리스트 동작을 안정적으로 유지하기 어렵습니다.

### UIKit은 통제할 수 있지만, 화면 코드는 무거워집니다

UIKit의 `UICollectionView`에서는 데이터 상태를 명시적으로 적용하고, diff 결과를 batch update로 반영할 수 있습니다. cell reuse와 delegate lifecycle도 드러나므로 병목을 측정하고 업데이트 전략을 조정할 수 있습니다.

반면 화면마다 data source와 delegate를 다시 구현하고, diff와 이벤트를 연결하면 코드가 늘고 동작이 분산됩니다.

### PopPangListKit은 선언과 렌더링 책임을 나눕니다

PopPangListKit은 리스트의 뼈대를 `UICollectionView`로 유지하면서 SwiftUI와 유사한 선언형 작성 경험을 제공합니다. 개발자는 `List`, `Section`, `Cell`을 선언하고, Core는 `UICollectionView`, Compositional Layout, DifferenceKit으로 diff, cell reuse, layout과 scroll event를 처리합니다.

UIKit `Component`와 SwiftUI `View`는 같은 `Section`, 같은 데이터 snapshot, 같은 업데이트 경로를 공유합니다. **즉, UIKit의 통제력은 유지하면서 화면마다 반복하던 연결 코드를 줄이는 것이 목적입니다.**

### 어떤 화면에 적합한가요?

| 선택 | 적합한 화면 | 감수할 점 |
|---|---|---|
| SwiftUI `List` | swipe, edit, selection 같은 플랫폼 기본 리스트 기능이 중요한 화면 | 여백, 배경, 구분선 같은 기본 규칙을 커스텀 디자인에서 우회해야 할 수 있음 |
...
- [설계 기준](#설계-기준)
- [빠른 시작](#빠른-시작)
- [SwiftUI 프로그램 스크롤](#swiftui-프로그램-스크롤)
- [Core 구조](#core-구조)
...
가변 크기 cell을 섞는 일반 가로 리스트는 `.continuous`를 사용합니다. `.none`은 가로 스크롤 없이 같은 가로 group을 배치합니다. 전체 조합 예제는 Demo 앱의 `example 4 - Home list`에서 확인할 수 있습니다.

### Supplementary 전체 폭 배경

Section inset은 Header·Footer 콘텐츠와 Cell의 정렬에 계속 적용됩니다. 고정 여부와 관계없이 supplementary의 배경만 collection view 전체 폭으로 확장하려면 `headerBackground(_:)` 또는 `footerBackground(_:)`를 사용합니다. SwiftUI Header와 UIKit `Component` Header 모두 같은 API와 렌더링 경로를 사용합니다.
...
.withSectionLayout(
    ...
    .headerPinToVisibleBounds(true) // 선택 사항
)
```

`headerPinToVisibleBounds`와 `footerPinToVisibleBounds`는 위치만 고정합니다. `headerBackground`와 `footerBackground`는 콘텐츠 frame이나 section inset을 바꾸지 않고, 스크롤·회전·화면 크기 변경에 맞춰 배경 frame만 다시 계산합니다.

## SwiftUI 프로그램 스크롤

`didScroll`은 현재 위치를 **관찰**하는 API입니다. 버튼 탭처럼 코드에서 위치를 바꾸려면 `@State`의 `ListProxy`를 `PopPangList(proxy:)`에 전달하세요.

기존 `PopPangListProxy`는 deprecated typealias로 남아 있어 기존 코드는 계속 컴파일됩니다. 새 코드는 `ListProxy`를 사용하세요.
...
`scrollOverlay`는 Proxy와 별개입니다. 라이브러리가 `isVisible` 상태를 관리하므로 화면에 `@State isTopAnchorVisible`, 별도 Observable 객체, `didScroll` 계산 코드를 둘 필요가 없습니다. 표시 상태가 바뀌어도 overlay 하위 View만 갱신하고 List snapshot을 다시 적용하지 않습니다. 숨김 상태의 overlay는 hit testing과 accessibility에서 제외됩니다.

| 표시 기준 | 의미 |
|---|---|
| `.relativeToViewport(1.5)` | 현재 viewport 높이의 1.5배를 세로로 스크롤한 뒤 표시합니다. |
| `.points(240)` | 240pt를 세로로 스크롤한 뒤 표시합니다. |

세로 offset은 `max(0, contentOffset.y - adjustedContentInset.top)`으로 계산합니다. 현재는 vertical scroll 기준이며, `didScroll`과 함께 사용해도 서로의 handler를 덮어쓰지 않습니다.

최상단으로 이동할 때는 다음처럼 사용합니다. `scrollToTop`은 safe area와 refresh control 등을 포함한 `adjustedContentInset.top`을 반영해 실제 콘텐츠 시작점으로 이동합니다.

```swift
listProxy.scrollToTop(animated: true)
```

`scrollToSection`의 ID는 `String`, `Int`, enum 등 모든 `Hashable` 값을 사용할 수 있습니다. 반환값은 명령이 수락되었는지 나타냅니다.

| 상황 | 반환값과 동작 |
|---|---|
| 연결된 List의 Cell이 있는 section ID | `true`, 최신 snapshot에서 찾은 첫 Cell로 이동 |
| List가 아직 mount되지 않았거나 제거된 뒤 | `false`, 명령을 대기시키지 않고 무시 |
| 존재하지 않는 section ID 또는 빈 section | `false`, 크래시 없이 무시 |
| diff 적용 뒤 section 순서가 변경된 경우 | 현재 적용된 최신 snapshot의 ID를 다시 찾아 이동 |

`position`에는 `.top`, `.centeredVertically`, `.bottom`, `.left`, `.centeredHorizontally`, `.right`를 사용할 수 있습니다. 대부분의 세로 리스트는 기본값인 `.top`을 사용하면 됩니다.
...
| `AnyComponent` | 서로 다른 `Component`를 하나의 Cell 모델로 다루는 타입 소거 래퍼 | 리스트 내부 저장과 diff 비교 시 |
| `Cell` | 컬렉션 뷰 아이템 모델 | 아이템 UI와 이벤트를 선언할 때 |
| `Section` | Cell, header, footer, layout을 묶는 모델 | 섹션별 화면 구조를 만들 때 |
| `List` | 화면 전체 snapshot과 리스트 이벤트 | 렌더링할 리스트 상태를 구성할 때 |
| `CollectionViewAdapter` | `List`와 `UICollectionView`를 연결하는 어댑터 | UIKit 화면에 리스트를 반영할 때 |
...
### Section별 update animation

리스트 전체가 `.animatedBatchUpdates`를 사용할 때 특정 Section 내부 변경만 애니메이션 없이 적용할 수 있습니다.

```swift
PopPangList {
    Section(id: "best") {
        For(bestPopups, id: \.popupUuid) { popup in
            BestPopupCell(popup: popup)
        }
    }
    .disablesUpdateAnimation()
    .withSectionLayout(bestLayout)

    Section(id: "grid") {
        For(gridPopups, id: \.popupUuid) { popup in
            GridPopupCell(popup: popup)
        }
    }
    .withSectionLayout(gridLayout)
}
```

`best`의 Cell, Header, Footer 변경은 non-animated batch update로 먼저 적용하고, `grid`를 포함한 나머지 변경은 이어지는 animated batch update로 적용합니다. UIKit에서 `List`와 `Section`을 직접 구성할 때도 같은 modifier를 사용할 수 있습니다.

Section 정책은 다음 우선순위와 범위를 가집니다.

- 리스트 전체가 `.nonanimatedBatchUpdates` 또는 `.reloadData`이면 전체 전략이 우선합니다.
- 기존 snapshot에도 같은 ID로 존재하는 Section 내부 변경에만 적용됩니다.
- Section 자체의 삽입, 삭제, 순서 이동은 리스트 전체 update strategy를 따릅니다.
- Section 높이가 바뀌면 아래 Section의 layout 위치는 애니메이션 없이 즉시 달라질 수 있습니다.
- 다른 Section이 `reloadData`되는 것을 막기 위해 section-scoped non-animated 단계에는 `batchUpdateInterruptCount` 기반 전체 reload를 적용하지 않습니다.
- 옵션을 사용하는 snapshot은 non-animated와 animated 두 번의 diff 단계로 나뉩니다. 업데이트 도중 들어온 최신 snapshot은 두 단계가 모두 끝난 뒤 적용됩니다.
...
개발자는 화면마다 `UICollectionViewCell`이나 `UICollectionReusableView`를 subclass로 만들 필요가 없습니다. 대신 Component를 작성하고 `Cell`, Header, Footer에 넣습니다. 리스트는 공통 렌더링 Cell이 처리하고, Component는 화면에 필요한 View와 데이터 갱신 규칙만 정의합니다.

`Component`의 생성, 갱신, Coordinator 계약은 `UIViewRepresentable`과 비슷합니다. UIKit Component와 SwiftUI View는 같은 Section, snapshot, 업데이트 경로를 공유하므로 화면을 SwiftUI로 전환할 때 리스트 구조와 갱신 로직을 다시 만들 필요가 없습니다.
...
| `.flexibleHeight(estimatedHeight:)` | 컨테이너에 맞춤 | Content에 맞춤 | 일반적인 세로 리스트 Cell |
| `.flexibleWidth(estimatedWidth:)` | Content에 맞춤 | 컨테이너에 맞춤 | 가로 리스트의 높이가 정해진 카드 |
...
`SwiftUIHostingComponent`는 SwiftUI View를 별도의 리스트 엔진으로 처리하지 않습니다. SwiftUI View를 기존 Component 계약에 맞게 변환하는 Adapter입니다.
...
| `ListProxy` | `scrollToTop`, `scrollToSection` | SwiftUI 버튼·액션에서 리스트 위치를 명령형으로 변경합니다. `PopPangListProxy`는 deprecated 호환 alias입니다. |
| `PopPangList` | `scrollOverlay` | 스크롤 위치를 기준으로 SwiftUI overlay의 표시 상태를 제공합니다. |
| `Section` | `withHeader`, `withFooter`, `withSectionLayout` | 보조 뷰와 Compositional Layout을 섹션에 붙입니다. |
| `Section` | `headerBackground`, `footerBackground` | Header·Footer 콘텐츠 정렬은 유지한 채 배경만 collection view 전체 폭으로 확장합니다. |
...
- 같은 Cell ID를 공유하는 Section과 Section별 update animation 비교 화면
...
PopPangListKit은 SwiftUI를 대체하지 않습니다. 단순하고 정적인 리스트는 SwiftUI `List`나 `LazyVStack`으로 충분합니다. 복잡한 리스트에서 업데이트 경로를 예측하고 튜닝해야 할 때 UICollectionView Core와 선언형 DSL을 함께 제공하는 것이 이 모듈의 역할입니다.
...
| 구분 | API | 설명 |
|---|---|---|
| 리스트 연결과 갱신 | `CollectionViewAdapter`, `adapter.apply(List)` | `UICollectionView`를 직접 만들고 원하는 시점에 새 List snapshot을 적용합니다. |
| UIKit Cell | `Cell(id:component:)` | `Component`로 `UIView` Cell을 렌더링합니다. |
| SwiftUI Cell | `Cell(id:item:content:)` | UIKit 리스트 안에서도 SwiftUI View를 Cell로 렌더링합니다. |
...
| Section Layout | `VerticalLayout`, `HorizontalLayout`, `VerticalGridLayout`, `DefaultCompositionalLayoutSectionFactory`, custom factory | 세로 리스트, 가로 리스트, 그리드, 기본 factory, custom Compositional Layout을 섞습니다. |
...
| Header / Footer 배경 | `.headerBackground`, `.footerBackground` | SwiftUI와 UIKit Component supplementary의 배경을 전체 폭으로 확장합니다. |
| Section update animation | `.disablesUpdateAnimation()` | 해당 Section 내부 변경을 non-animated 단계로 먼저 적용합니다. Section 자체의 삽입·삭제·이동은 전체 전략을 따릅니다. |
...
| 새로고침과 페이지네이션 | `onRefresh`, `onReachEnd` | RefreshControl과 리스트 끝 감지를 연결합니다. |
...
| 구분 | API | 설명 |
|---|---|---|
| 리스트 연결과 갱신 | `PopPangList { ... }` | SwiftUI `body`가 다시 계산되면 새 List snapshot을 자동 적용합니다. |
...
| Header / Footer 배경 | `.headerBackground`, `.footerBackground` | 콘텐츠의 section inset은 유지하고 supplementary 배경만 전체 폭으로 확장합니다. |
| Section update animation | `.disablesUpdateAnimation()` | 지정한 Section 내부 변경만 애니메이션 없이 적용하고 나머지 Section은 animated diff를 유지합니다. |
...
| 프로그램 스크롤 | `PopPangList(proxy:)`, `ListProxy`, `scrollToTop`, `scrollToSection` | `@State` Proxy로 특정 section 또는 실제 콘텐츠 최상단으로 이동합니다. `PopPangListProxy`도 deprecated 상태로 호환됩니다. |
| 스크롤 기반 overlay | `.scrollOverlay(alignment:visibleWhen:)` | 앱 상태 없이 vertical scroll 위치 기준 overlay의 표시 상태와 hit testing을 관리합니다. |
